...
import (
	...
	"github.com/shawlyelshalawy/logsync/internal/log"
...
func setupTest(t *testing.T, fn func(*Config)) (
	...
	newClient := func(crtPath, keyPath string) (api.LogClient, *grpc.ClientConn) {
		...
		conn, err := grpc.NewClient(l.Addr().String(), grpc.WithTransportCredentials(tlsCreds))
		require.NoError(t, err)

		return api.NewLogClient(conn), conn
	}

	rootClient, rootConn := newClient(config.RootClientCertFile, config.RootClientKeyFile)
	nobodyClient, nobodyConn := newClient(config.NobodyClientCertFile, config.NobodyClientKeyFile)
